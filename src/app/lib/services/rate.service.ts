...
export class RateService {
  ...
  private baseurl = 'https://proyecto-web-freelancer.web.app/api/v0/rate';
  ...
  getAll() {
    return this.http.get<Rate[]>(this.baseurl, {headers: this.headers}).pipe(catchError(this.handleError));
  }

  get(id: any) {
    return this.http.get<Rate>(this.baseurl.concat('?id=', id), {headers: this.headers}).pipe(catchError(this.handleError));
  }
  ...
  pag(size: number, page: number) {
    return this.http.get<Rate[]>(this.baseurl.concat('/', size.toString(), '/', page.toString()), {headers: this.headers}).pipe(catchError(this.handleError));
  }
